...
- (void)startListening:(id<RIOInterfaceDelegate>)aListener {
	self.delegate = aListener;
	[self createAUProcessingGraph];
	[self initializeAndStartProcessingGraph];	
}
					   ...
					   AudioBufferList				*ioData)
{
	...
	if (read > inNumberFrames) {
        ...
        if ([THIS->delegate respondsToSelector:@selector(frequencyChangedWithValue:)]) {
            [THIS->delegate frequencyChangedWithValue:bin*(THIS->sampleRate/bufferCapacity)];
